*If not debugging:*


  * *Anchor: <a name="0"></a>0*


  * `💬`: "Grovyle, who came from the future to prevent the planet's paralysis..."


  * `💬`: "Collected the Time Gears..."


  * `💬`: "And together with  and ..."


  * `💬`: "Made his way to Dialga's Temporal Tower."


  * `💬`: "However..."


  * * * *


  * `Dusknoir Normal`

  * `Dusknoir`: "I won't... You'll never change history!"


  * * * *


  * `Hero Shouting`

  * `Hero Name`: "(Waah!)"


  * * * *


  * `Grovyle Pain`

  * `Grovyle`: "Urrgggghhh!"


  * `Hero Surprised` 〽

  * `Hero Name`: "(Grovyle!)"


  * `Dusknoir Normal`

  * `Dusknoir`: "So you shielded , Grovyle?!"


  * `Dusknoir Normal`

  * `Dusknoir`: "That's taken a heavy toll on you!"


  * `Dusknoir Normal`

  * `Dusknoir`: "Very well! You're going down first!"


  * `Grovyle Pain`

  * `Grovyle`: "Uwroh... Wrooooooooh!"


  * `Dusknoir Normal` 〽

  * `Dusknoir`: "Wh-what are you doing?!"


  * `Grovyle Pain`

  * `Grovyle`: "Urrggh... Dusknoir!"


  * `Grovyle Pain`

  * `Grovyle`: "I'm... I'm taking you with me..."


  * `Grovyle Pain`

  * `Grovyle`: "We're going back to the future!"


  * `Dusknoir Normal` 〽

  * `Dusknoir`: "Wha..."


  * `Hero Surprised` 〽

  * `Hero Name`: "(What?!)"


  * `Grovyle Normal`

  * `Grovyle`: "! It's up to you now!"


  * `Dusknoir Normal` ❗

  * `Dusknoir`: "Gah! Those are...!"


  * `Hero Surprised` ❗

  * `Hero Name`: "(Time Gears!)"


  * `Partner Name`: "OK, team! The Rainbow Stoneship's set to go!"


  * `Partner Surprised`

  * `Partner Name`: "Wha...what's happening?!"


  * `Grovyle Normal`

  * `Grovyle`: "! This is it for me!"


  * `Grovyle Normal`

  * `Grovyle`: "I'm taking Dusknoir...back to the future!"


  * `Partner Surprised` 〽

  * `Partner Name`: "Huh?! Wh-what?!"


  * `Grovyle Normal`

  * `Grovyle`: "I can never come back here again."


  * `Grovyle Normal`

  * `Grovyle`: "Watch 's back... Promise me!"


  * `Partner Teary-Eyed` 💦

  * `Partner Name`: "B-but...!"


  * `Partner Teary-Eyed`

  * `Partner Name`: "Take your place, Grovyle...? I can't!"


  * `Grovyle Normal`

  * `Grovyle`: "You must and you will. Because you can, ."


  * `Grovyle Normal`

  * `Grovyle`: "You two are...the greatest of combinations."


  * `Dusknoir Normal` 〽

  * `Dusknoir`: "Gwoh! Unhand me! Release me!"


  * `Grovyle Normal`

  * `Grovyle`: "We're almost there! Quiet!"


  * `Grovyle Normal`

  * `Grovyle`: "...!"


  * `Grovyle Normal` ❗

  * `Grovyle`: "Take care, ."


  * `Grovyle Normal`

  * `Grovyle`: "I was lucky to have known you."


  * `Grovyle Normal`

  * `Grovyle`: "Though the parting hurts... The rest is in your hands!"


  * `Hero Surprised` ❗

  * `Hero Name`: "(Grovyle!)"


  * `Grovyle Normal`

  * `Grovyle`: "Sorry for the holdup, Dusknoir!"


  * `Dusknoir Normal` 〽

  * `Dusknoir`: "Gwaaaaaaah!"


  * * * *


  * `Dusknoir Normal`

  * `Dusknoir`: "After entrusting his mission to his friends..."


  * `Dusknoir Normal`

  * `Dusknoir`: "Grovyle returned to the future."


  * * * *


  * `Partner Teary-Eyed`

  * `Partner Name`: "Ugh... Grovyle... Why...?"


  * `Hero Sad`

  * `Hero Name`: "(Grovyle...)"


  * * * *


  * `Hero Sad`

  * `Hero Name`: "Yet  and  were..."


  * `Hero Sad`

  * `Hero Name`: "Desperately trying to contain their sorrow..."


  * * * *


  * `Partner Determined`

  * `Partner Name`: "Hey, !"


  * `Partner Determined`

  * `Partner Name`: "We have to do this for Grovyle's sake! Let's go to Temporal Tower!"


  * * * *


  * `Partner Determined`

  * `Partner Name`: "Filled with that sentiment..."


  * `Partner Determined`

  * `Partner Name`: "They set off on a final adventure..."


  * `Partner Determined`

  * `Partner Name`: "But then..."


  * `Partner Determined`

  * `Partner Name`: "After having returned to the future with Dusknoir..."


  * `Partner Determined`

  * `Partner Name`: "Grovyle..."


*Else:*


  * `Partner Determined`

  * `Partner Name`: "Start from where?"


  * *If the player chooses "[M:D2]Beginning":*

  *   * *Jump to [anchor 0](#0)*


