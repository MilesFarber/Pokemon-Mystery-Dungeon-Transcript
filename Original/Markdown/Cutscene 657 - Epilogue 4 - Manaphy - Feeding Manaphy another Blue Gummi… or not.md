`Manaphy Normal`

`Manaphy`: "Goomi! Goomi!"


>Would you like to give a Blue Gummi to Manaphy?

*If the player chooses "Yes":*

  * *Anchor: <a name="0"></a>0*


  * * * *


  * * * *

>
>`Hero Name` gave a Blue Gummi to Manaphy.

  * `Manaphy Joyous` ❗

  * `Manaphy`: "Goomi! ♪"


  * `Manaphy Normal`

  * `Manaphy`: "Chew, chew..."


  * `Partner Normal`

  * `Partner Name`: "Is it tasty? Er... Goomi tasty?"


  * `Manaphy Normal` ❓

  * `Manaphy`: "Tasty?"


  * `Partner Normal`

  * `Partner Name`: "Yes. Tasty?"


  * `Manaphy Happy` 🔆

  * `Manaphy`: "Tasty! ♪ Goomi tasty! ♪"


  * `Partner Happy`

  * `Partner Name`: "Ha! The little cutie's going to grow up in no time at this rate!"


  * `Manaphy Stunned`

  * `Manaphy`: "....................."


  * `Partner Normal` ❓

  * `Partner Name`: "Oh? Is something the matter?"


  * `Manaphy Sigh`

  * `Manaphy`: "...Yawn... Snuffle..."


  * `Partner Happy` ❗

  * `Partner Name`: "Ha! The little cutie must be full. He's gone to sleep now."


  * `Partner Normal`

  * `Partner Name`: "We should let him sleep."


  * `Partner Normal`

  * `Partner Name`: "Sleep tight, Manaphy."


  * * * *


  * `Partner Normal`

  * `Partner Name`: "The next morning..."


  * * * *


  * `Partner Normal` ❗

  * `Partner Name`: "Good morning, `Hero Name`!"


  * `Manaphy Happy`

  * `Manaphy`: "Hi!"


  * `Partner Happy`

  * `Partner Name`: "Good morning, Manaphy."


  * `Manaphy Happy`

  * `Manaphy`: "Morning! `Hero Name`! `Partner Name`!"


  * `Partner Surprised` 〽

  * `Partner Name`: "Wow! The little cutie greeted us by name!"


  * `Partner Surprised`

  * `Partner Name`: "That's awesome, Manaphy!"


  * `Manaphy Joyous`

  * `Manaphy`: "Awesome!"


  * `Partner Normal`

  * `Partner Name`: "Well, `Hero Name`. What should we do with him today?"


  * `Partner Normal`

  * `Partner Name`: "Eating and sleeping all the time doesn't seem healthy enough..."


  * `Partner Normal` ❗

  * `Partner Name`: "I know! Let's take the little cutie for a walk at the beach!"


  * `Manaphy Happy`

  * `Manaphy`: "Walk! ♪"


  * * * *


  * *Anchor: <a name="3"></a>3*


>`Hero Name` doesn't have a Blue Gummi...

*If the player chooses "No":*

  * `Manaphy Crying`

  * `Manaphy`: "Weh... Weh... Waaaah!"


  * `Partner Surprised` 💦

  * `Partner Name`: "Oh, no no... Please don't cry!"


  * `Partner Surprised`

  * `Partner Name`: "Don't you cry, cutie. We'll be back soon with a tasty Blue Gummi for you."


